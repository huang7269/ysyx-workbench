...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  IFDEF(CONFIG_FTRACE,{if(rd == 1) call_ftrace(s->pc,s->dnpc);}));
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4;s->dnpc = (src1 + imm)& ~1u;
  IFDEF(CONFIG_FTRACE,{if(s->isa.inst == 0x00008067)
    {ret_ftrace(s->pc);}
    else if(rd == 1)
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = (src1 < imm || src1 == 0));
  ...
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (int32_t)src1 >> (imm - 1024));
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", ram    , R, R(rd) = (int)src1 % (int)src2);
